...
\usepackage{setspace}
\setcounter{secnumdepth}{1} % keep section counter for theorem numbering
\makeatletter
...
\newtheoremstyle{teoremaBreak}
  {6pt}{6pt}
  {\normalfont}{}{
  \bfseries}{.}{\newline}{\thmname{#1}\thmnumber{ #2}\thmnote{ (#3)}}
\theoremstyle{teoremaBreak}
...
\section{Capítulo 1. Introducción}\label{capítulo-1.1-introducción}

\subsection{\texorpdfstring{1.1 Contexto del proyecto Conductoma y
...
ciertos conocimientos de calculo diferencial e integral, así como 
de álgebra lineal y estadística, ya que se profundizará únicamente los conceptos directamente
relacionados con los proyectos Conductome y Project42.

El principal objetivo de este capítulo es presentar y demostrar el teorema central 
del límite, que será utilizado posteriormente para justificar la construcción de las 
métricas Epsilon y Score, así como para entender el comportamiento de estas métricas.
...
muchas variables (multidimensionales) . Después se definen conceptos fundamentales 
como axiomas de probabilidad, sigma álgebra, probabilidad condicional, independencia de eventos, 
variables aleatorias, esperanza, varianza, función generadora de momentos, entre otros.

Se presentan algunos resultados importantes como el teorema de Bayes así como 
propiedades de variables aleatorias, de esperanza y varianza, otro resultado importante es el de 
la continuidad de las funciones generadoras de momentos, para así poder probar el teorema del límite central.
...
``bueno'' para clasificar independientemente de sus datos.
...
sobre el cual se fundamentan diferentes resultados. 

Decimos que $\mathbb{P}$ es una
medida de probabilidad si cumple con los siguientes axiomas.
...
Ahora definimos una sigma álgebra de subconjuntos de $\mathbb{R}$ que resulta ser de suma importancia para la probabilidad, 
pues esta sigma-álgebra nos va a ayudar a que las funciones de probabilidad estén bien definidas.
Queda fuera del alcanze de este trabajo el desarrollo de la teoría de la medida, y nos limitaremos a 
mencionar que la idea de pedir $P(X \in B)$ con $B \subset \mathbb{R}$ no siempre tiene sentido, 
pues existen conjuntos "no medibles" para los cuales no tiene sentido asignar una probabilidad, la sigma álgebra 
de Borel nos da un dominio seguro donde la medida de probabilidad existe y es consistente.

Sea $\Omega$ un conjunto, decimos que $\mathcal{B}(\mathbb{R})$ es la sigma-álgebra de Borel 
si es la mínima sigma=álgebra de subconjuntos de $\mathbb{R}$ que contiene a todos los intervalos de la 
forma $(-\infty, a]$ con $a \in \mathbb{R}$. Es decir,
$$\mathcal{B}(\mathbb{R}) = \sigma(\{(-\infty, a] \mid a \in \mathbb{R}\}).$$

Donde $\sigma$ denota que se está tomando la mínima sigma-álgebra generada por la colección 
$\{(-\infty, a] \mid a \in \mathbb{R}\}$, donde "mínimo" hace referencia a que si $F$ es una sigma-álgebra 
de subconjuntos de $\mathbb{R}$ tal que $\{(-\infty, a] \mid a \in \mathbb{R}\} \subseteq F$, 
entonces $\mathcal{B}(\mathbb{R}) \subseteq F$.
\end{definicion}

Usualmente se refiere a los elementos de $\mathcal{B}(\mathbb{R})$ como borelianos de $\mathbb{R}$.

Podemos notar que los borelianos de $\mathbb{R}$ incluyen a los intervalos de la forma 
$[a, b)$, $(a, b]$, $(a, b)$, $[a, b]$, $(-\infty, a)$, $(a, \infty)$, ${{a}}$, $\mathbb{N}$, $\mathbb{Q}$, 
$\mathbb{R}$, $\mathbb{Z}$, $\mathbb{R} \setminus \mathbb{Q}$, entre otros.
...
\mathcal{P}(\Omega)$, entonces cualquier función $X:\Omega \to \mathbb{R}$ cumple con la
...
Como $F = \mathcal{P}(\Omega)$, se sigue que X es una variable
...
numerable de resultados. $F$ es una sigma-álgebra de subconjuntos de $\Omega$
y $\mathbb{P}$ es una medida de probabilidad sobre $F$.
...
Sea ($\Omega, F, \mathbb{P}$) un espacio de probabilidad y sea $X$ una variable aleatoria discreta con valores posibles $x_1, x_2, \ldots$ Definimos la función de 
probabilidad de $X$, denotada por $f: \mathbb{R} \to \mathbb{R}$ como:
\[
f(x)=
\begin{cases}
...
Bajo esta definición, la probabilidad de un evento $A$ con una variable aleatoria como la usada 
arriba se reduce al calculo de una suma.
...
Sea $X$ una variable aleatoria, definimos la función de distribución de $X$, denotada por
 $F(x): \mathbb{R} \to [0,1]$ como:
...
también se le conoce como valor esperado o valor promedio y usualmente se le denota por $\mu$.
\end{definicion}
...
a su esperanza. Usualmente denotamos a la varianza por $\sigma^2$ y a la raiz cuadrada de la varianza 
$ \sigma = \sqrt{\text{Var}(X)}$ se le conoce como desviación estándar.
\end{definicion}

\begin{proposicion}
Sean $X$ y $Y$ dos variables aleatorias independientes y sean $f(x)$ y $g(x)$ dos funciones tales que el 
producto $f(X)g(Y)$ es una variable aleatoria con esperanza finita, entonces:
$$\mathbb{E}[f(X)g(Y)] = \mathbb{E}[f(X)] \cdot \mathbb{E}[g(Y)]$$ 
\end{proposicion}

\textbf{Demostración.}

Usando la independencia de $X$ y $Y$ y la definición de esperanza, tenemos que:
\begin{align*}
\mathbb{E}[f(X)g(Y)] &= \sum_{x} \sum_{y} f(x)g(y) \mathbb{P}(X = x, Y = y) \\
&= \sum_{x} \sum_{y} f(x)g(y) \mathbb{P}(X = x) \cdot \mathbb{P}(Y = y) \\
&= \left(\sum_{x} f(x) \mathbb{P}(X = x)\right) \cdot \left(\sum_{y} g(y) \mathbb{P}(Y = y)\right) \\
...
\begin{proposicion}
Sean $X$ y $Y$ dos variables aleatorias independientes con funciones generadoras de momentos $M_X(t)$ y $M_Y(t)$ 
respectivamente, entonces:
$$M_{X + Y}(t) = M_X(t) \cdot M_Y(t)$$
\end{proposicion}

\textbf{Demostración.}
Usando la definición de función generadora de momentos y la independencia de $X$ y $Y$ tenemos que:
\begin{align*}
...
\begin{proposicion}
Sea X una variable aleatoria con función generadora de momentos $M(t)$, definida para valores de 
$t$ en un intervalo abierto que contiene a cero. Entonces, todos los momentos de $X$ existen y además 
$M(t)$ se puede escribir como la siguiente serie de potencias:
$$M(t) = \sum_{n=0}^{\infty} \dfrac{t^n}{n!} \mathbb{E}[X^n]$$
\end{proposicion}

\textbf{Demostración.}

Para demostrar esta proposición, vamos a usar desarrollar la serie de Taylor de $e^{tX}$ alrededor 
del cero y evaluada en $tX$, con $t \in (-\delta, \delta)$ para algún $\delta > 0$. Esto es:
$$e^{tX} = \sum_{n=0}^{\infty} \dfrac{(tX)^n}{n!} = \sum_{n=0}^{\infty} \dfrac{t^n}{n!} X^n$$

Ahora, tomando la esperanza de ambos lados tenemos:
\begin{align*}
M(t) &= \mathbb{E}[e^{tX}] \\
&= \mathbb{E}\left[\sum_{n=0}^{\infty} \dfrac{t^n}{n!} X^n\right] \\
&= \sum_{n=0}^{\infty} \mathbb{E}\left[ \dfrac{t^n}{n!} X^n\right] \\
&= \sum_{n=0}^{\infty} \dfrac{t^n}{n!} \mathbb{E}[X^n]
\end{align*}

En el penúltimo paso podemos hacer el intercambio de la suma y la esperanza porque 
la serie de potencias converge absolutamente para $t \in (-\delta, \delta)$, 
lo cual implica que la serie de potencias de $e^{tX}$ también converge absolutamente para 
$t \in (-\delta, \delta)$, y por lo tanto, podemos usar el **teorema de convergencia dominada** para 
justificar el intercambio de la suma y la esperanza.

\begin{teorema}[Continuidad de las funciones generadoras de momentos]
Sea $X_1, X_2, \ldots$ una sucesion de variables aleatorias con funciones generadoras de momentos
 $M_1, M_2, \ldots$ respectivamente. Sea $X$ una variable aleatoria con función generadora de momentos 
 $M$, si sucede que para cada $t \in (-s, s)$ con $s > 0$. 
$$\lim_{n \to \infty} M_n(t) = M(t)$$
Entonces, para todo punto de $x$ donde la función de distribución de X, $F_X(x)$ es continua se tiene que:
$$\lim_{n \to \infty} F_{X_n}(x) = F_X(x)$$
A esta propiedad se le llama convergencia en distribución de $X_n$ a $X$ y se denota 
por $X_n \xrightarrow{d} X$.
\end{teorema}
...
Este teorema postula que si una sucesión de variables $X_1, X_2, \ldots , X_n$ son independientes 
e idénticamente distribuidas con media $\mu$ y varianza finita $\sigma^2$, entonces la variable aleatoria 
definida por $$Z = \dfrac{\sum_{i=1}^n X_i - n\mu}{\sigma \sqrt{n}}$$ converge en distribución a una 
variable aleatoria normal estándar, es decir, $Z \sim N(0,1)$.
\end{teorema}

\textbf{Demostración.}

Esta demostración esta basada en la propuesta por Lindeberg-Lévy, y la idea es calcular la función generadora 
de momentos de $Z$ para después calcular el límite de ésta cuando $n$ tiene a infinito y hacer uso del teorema 
de continuidad de las funciones generadoras de momentos para probar la convergencia en distribución.

Primero veamos que se cumple lo siguiente:
$$M_Z(t) = \left[M_{\dfrac{X - \mu}{\sigma}}(\dfrac{t}{\sqrt{n}})\right]^n$$
...
\begin{align*}
M_Z(t) &= \mathbb{E}[e^{tZ}] \\
&= \mathbb{E}\left[e^{t\left(\dfrac{\sum_{i=1}^n X_i - n\mu}{\sigma \sqrt{n}}\right)}\right] \\
&= \mathbb{E}\left[e^{\dfrac{t}{\sigma \sqrt{n}}\sum_{i=1}^n (X_i - \mu)}\right] \\
&= \mathbb{E}\left[\prod_{i=1}^n e^{\dfrac{t}{\sigma \sqrt{n}}(X_i - \mu)}\right] \\
&= \prod_{i=1}^n \mathbb{E}\left[e^{\dfrac{t}{\sigma \sqrt{n}}(X_i - \mu)}\right] \\
&= \prod_{i=1}^n M_{\dfrac{X - \mu}{\sigma}}\left(\dfrac{t}{\sqrt{n}}\right) \\
&= \left[M_{\dfrac{X - \mu}{\sigma}}\left(\dfrac{t}{\sqrt{n}}\right)\right]^n
\end{align*}

Ahora usaremos la propiedad de expresión en potencias de $M_{\dfrac{X - \mu}{\sigma}}(t)$.

\begin{align*}
M_Z(t) &= \mathbb{E}[e^{tZ}] \\
&= \left[M_{\dfrac{X - \mu}{\sigma}}\left(\dfrac{t}{\sqrt{n}}\right)\right]^n \\
&= (1 + \dfrac{t}{\sqrt{n}} \mathbb{E}(\dfrac{X - \mu}{\sigma}) + \dfrac{t^2}{2n} \mathbb{E}((\dfrac{X - \mu}{\sigma})^2) + \ldots)^n \\
&= (1 + \dfrac{t^2}{2n} + o(\dfrac{t^2}{n}))^n
\end{align*}

Notemos que en el último paso hemos usado:

$$\mathbb{E}\left(\dfrac{X - \mu}{\sigma}\right) = 0 \text{ pues } X \text{ está centrada en } \mu$$

$$\mathbb{E}\left(\left(\dfrac{X - \mu}{\sigma}\right)^2\right) = 1 \text{ pues es la varianza de la variable estandarizada}$$
...
\begin{align*}
\lim_{n \to \infty} M_Z(t) &= \lim_{n \to \infty} (1 + \dfrac{t^2}{2n} + o(\dfrac{t^2}{n}))^n \\
&= \lim_{n \to \infty} \left[\left(1 + \dfrac{t^2}{2n} + o(\dfrac{t^2}{n})\right)^{\dfrac{2n}{t^2}}\right]^{\dfrac{t^2}{2}} \\
&= e^{\dfrac{t^2}{2}}
\end{align*}

Para justificar este límite, nótese que en la segunda línea reescribimos $(1 + \frac{t^2}{2n} + o(\frac{t^2}{n}))^n$ en la forma $(u^{k})^{1/k}$ donde $u = (1 + \frac{t^2}{2n} + o(\frac{t^2}{n}))$ y $k = \frac{2n}{t^2}$. Al tomar el límite, la base $u \to 1$ pero el exponente $k \to \infty$, por lo que se aplica el límite fundamental:

$$\lim_{k \to \infty} \left(1 + \dfrac{x}{k} + o\left(\dfrac{x}{k}\right)\right)^k = e^x$$

En nuestro caso, $x = \frac{t^2}{2}$, lo que produce $e^{t^2/2}$.

Así, llegamos a que la función generadora de momentos de $Z$ converge a $$M_Z(t) = e^{t^2/2}$$
La cual coincide con la función generadora de momentos de una variable aleatoria normal estándar $N(0,1)$.
Por lo tanto, usando el teorema de continuidad de las funciones generadoras de momentos, hemos probado que 
$$Z = \dfrac{\sum_{i=1}^n X_i - n\mu}{\sigma \sqrt{n}} \xrightarrow{d} N(0,1)$$


\begin{definicion}
\textbf{Odds Ratio.}

Sean $X$ y $C$ dos vectores aleatorios, le llamamos la proporción posterior de $C_1$ respecto a $C_2$ a la proporción
de densidad posterior $\mathbb{P}(C \mid X)$ evaluada en los puntos $C_1$ y $C_2$, esto es:
...
Sean $X_i$ y $C$ variables aleatorias, donde $C$ es la clase de interés.

Definimos la hipótesis nula $H_0: X_i \perp C$.

Sea $K_i$ una variable aleatoria que representa el número de observaciones con
...
Por lo tanto, $\mathbb{E}(K_i) = N_{X_i}\,\mathbb{P}(C)$ y $\operatorname{Var}(K_i) = N_{X_i}\,\mathbb{P}(C)\,(1 -
\mathbb{P}(C))$.
...
\[
\varepsilon = \dfrac{{K_i} - \mathbb{E}(K_i)}{\sigma}
\]

Por notación, consideramos $K_{i} = N_{CX_i}$.

Notemos que como $K_i \sim \text{Binomial}(N_{X_i}, p)$, $K_i$ se
...
Así, cuando $N_{X_i}$ tiende a infinito podemos aplicar el \textbf{teorema central del límite}, el cual dice
que:

$$\dfrac{\sum_{t = 1}^{N_{X_i}} Z_t - N_{X_i} \cdot \mathbb{E}(Z_t)}{\sqrt{N_{X_i} \cdot \operatorname{Var}(Z_t)}} \sim N(0,1)$$

Veamos que,
$$\dfrac{\sum_{t = 1}^{N_{X_i}} Z_t - N_{X_i} \cdot \mathbb{E}(Z_t)}{\sqrt{N_{X_i} \cdot \operatorname{Var}(Z_t)}}
= \dfrac{K_i - N_{X_i} \cdot p}{\sqrt{N_{X_i} \cdot p \cdot (1 - p)}} = \varepsilon$$
...
cuando $N_{X_i}  \to \infty$.



En la práctica, basta considerar que $N_{X_i}$ es suficientemente grande (por ejemplo, $N_{X_i} > 30$), 
